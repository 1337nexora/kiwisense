...
function Library:GetConfigs(Folder)
    ...
    if isfolder(Folder) then
        for _, file in ipairs(listfiles(Folder)) do
            if file:match("%.json$") and not file:match("AutoLoad") then
...
function Library:SaveConfig(Folder, Name)
    ...
    for flag, val in pairs(self.Flags) do
        if not tostring(flag):match("^UI_") then
            local sval = val
            if typeof(val) == "Color3" then
                sval = { R = val.R, G = val.G, B = val.B, IsColor3 = true }
            elseif typeof(val) == "EnumItem" then
                sval = { Name = val.Name, EnumType = tostring(val.EnumType), IsEnum = true }
            end
            data[flag] = sval
        end
    end
        ...
        if success and type(decoded) == "table" then
            for flag, val in pairs(decoded) do
                ...
                if type(val) == "table" then
                    if val.IsColor3 then
                        parsed = Color3.new(val.R, val.G, val.B)
                    elseif val.IsEnum then
                        local enumType = tostring(val.EnumType):match("%.([^%.]+)$") or "KeyCode"
                        parsed = Enum[enumType][val.Name]
                    end
                end
                if self.SetFlags and self.SetFlags[flag] then
                    pcall(function() self.SetFlags[flag](parsed) end)
                end
            end
            return true, "Success"
        end
...
function Library:BuildConfigSection(Tab, Folder)
    local Section = Tab:Section({ Name = "Configuration", Side = 1 })
    
    local ConfigList = Section:Dropdown({
        Name = "Config List",
        Items = self:GetConfigs(Folder),
        Callback = function(v) self.SelectedConfig = v end
    })
    
    local ConfigName = Section:Textbox({
        Name = "Config Name",
        Default = "",
        Placeholder = "Name",
        Callback = function(v) self.InputName = v end
    })
    
    Section:Button({
        Name = "Save Config",
        Callback = function()
            local name = self.InputName
            if not name or name == "" then name = self.SelectedConfig end
            if name and name ~= "" and name ~= "Select Config" then
                self:SaveConfig(Folder, name)
                pcall(function() ConfigList:SetValues(self:GetConfigs(Folder)) end)
            end
        end
    })
    ...
    Section:Button({
        ...
        Callback = function()
            ...
            if not name or name == "" then name = self.SelectedConfig end
            if name and name ~= "" and name ~= "Select Config" then
                self:LoadConfig(Folder, name)
            end
        end
    })
end

function Library:BuildPlayersSection(Tab)
    local Section = Tab:Section({ Name = "All Players", Side = 1 })
    local Players = game:GetService("Players")
    
    local function getPlayerNames()
        local list = {}
        for _, plr in ipairs(Players:GetPlayers()) do
            local n = plr.DisplayName
            if plr.UserId == 25099723 then n = "[🪐] " .. plr.DisplayName end
            table.insert(list, n .. " (@" .. plr.Name .. ")")
        end
        return list
    end
    ...
    local PlayerDropdown = Section:Dropdown({
        ...
        Callback = function(v) self.SelectedPlayer = v end
    })
